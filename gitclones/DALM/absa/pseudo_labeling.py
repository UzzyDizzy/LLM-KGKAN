...
def train(args, logger):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step in range(train_steps):
            source_batch, target_batch = next(source_iter), next(target_iter)
            ...
            # target domain is UNLABELED: get BERT hidden states directly (no CRF, no fake loss)
            target_bert_out = model.bert(target_input_ids, attention_mask=target_input_masks)[0]
            target_output = model.dropout(target_bert_out)
            # decode to get predicted aspect positions for the MMD mask (detached, no grad)
            with torch.no_grad():
                target_logits = model.classifier(target_output.detach())
                target_pred_raw = model.crf.decode(target_logits, mask=target_input_masks.bool())
            max_len = target_input_ids.size(1)
            target_pred_labels = torch.zeros(target_input_ids.size(0), max_len, dtype=torch.long, device=target_input_ids.device)
...
def pseudo_labeling(args, logger):
    ...
    # load model
    model = torch.load(os.path.join(args.output_dir, f"model.pt"), weights_only=False)
    model.cuda()
    model.eval()
