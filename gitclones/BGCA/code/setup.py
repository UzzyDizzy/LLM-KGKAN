...
def init_args():
    ...
    dataset_name = os.path.basename(os.path.normpath(args.dataset)) if ("/" in args.dataset or "\\" in args.dataset) else args.dataset
    output_dir = f"../outputs/{args.task}/{dataset_name}"
...
def prepare_pairs(args):
    # Check if dataset is a directory path for a specific pair
    if "/" in args.dataset or "\\" in args.dataset:
        basename = os.path.basename(os.path.normpath(args.dataset))
        if "-" in basename:
            src, tgt = basename.split("-")
            return {src: [tgt]}
