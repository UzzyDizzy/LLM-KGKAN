...
def train(args, tokenizer, model, train_dataset, task, epochs, lr, bs, acc_step=None, save_ckpt=False, save_last=False):
    ...
    n_gpus = args.n_gpu if isinstance(args.n_gpu, int) else len(args.n_gpu) if hasattr(args, "n_gpu") and args.n_gpu else 1
    t_total = (
        (len(train_dataloader.dataset) // (bs * max(1, n_gpus)))
        // acc_step
        * float(epochs)
    )
