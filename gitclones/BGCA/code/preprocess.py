...
def write_generation(file_paths, split="train", data_dir=None, do_write=True, nrows=None, mode='aste'):
    ...
    for path in file_paths:
        ...
        if mode == 'aste':
            ...
            if split in ["dev", "test"]:
                domain = re.search(rf"(\w+)[_-]{split}\.txt", path).group(1)
...
def preprocess(dataset_dir=None, data_dir=None, source=None, targets=None, do_write=True, nrows=None, unlabel=False, mode='uabsa'):
    ...
    if "/" in (dataset_dir or "") or "\\" in (dataset_dir or ""):
        if mode == 'uabsa':
...
def prepare_raw_data(args):
    ...
    if args.task in task_map:
        if "/" in args.dataset or "\\" in args.dataset:
            mode, _ = task_map[args.task]
            dataset_dir = args.dataset
            # Override file formats since convert_data.py names them simply "train.txt", etc.
            if mode == 'uabsa' or mode == 'aste':
